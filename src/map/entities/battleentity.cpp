...
int16 CBattleEntity::GetWeaponDelay(bool tp)
{
    ...
    if (auto* weapon = dynamic_cast<CItemWeapon*>(m_Weapons[SLOT_MAIN]))
    {
        ...
        if (!tp)
        {
            ...
            int16 hasteMagic   = std::clamp<int16>(getMod(Mod::HASTE_MAGIC), -10000, 5000);  // 43.75% cap -- handle 100% slow for weakness
            int16 hasteAbility = std::clamp<int16>(getMod(Mod::HASTE_ABILITY), -2500, 3000); // 25% cap
            int16 hasteGear    = std::clamp<int16>(getMod(Mod::HASTE_GEAR), -2500, 3000);    // 25%
        ...
        MinimumDelay -= (uint16)(MinimumDelay * 0.97);
