...
local function applyRuneEnhancement(effectType, player)
    ...
    --local meritBonus    =  player:getMerit(xi.merit.MERIT_RUNE_ENHANCE) -- 2 more elemental resistance per merit for a maximum total of (2*5) = 10 (power of merit is 2 per level)    rune_enchantment_effect
    local meritBonus    =  player:getMerit(xi.merit.rune_enchantment_effect) -- 2 more elemental resistance per merit for a maximum total of (2*5) = 10 (power of merit is 2 per level)    rune_enchantment_effect
...
xi.job_utils.rune_fencer.onSwordplayEffectTick = function(target, effect)
    ...
    if power < maxPower then
        ...
        if tickPower > 0 then
            ...
            target:addMod(xi.mod.MEVA, tickPower)
...
local function calculateSwipeLungeDamage(player, target, skillModifier, gearBonus, numHits, multipliers)
    local damage = math.floor(skillModifier * (3 * numHits + (gearBonus / 100)))
